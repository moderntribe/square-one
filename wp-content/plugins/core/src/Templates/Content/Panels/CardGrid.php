...
class CardGrid extends Panel {
	...
	public function get_title(): string {
		...
		if ( ! empty( $this->panel_vars[ CardGridPanel::FIELD_TITLE ] ) ) {
			$title = the_panel_title( esc_html( $this->panel_vars[ CardGridPanel::FIELD_TITLE ] ), 'site-section__title h2', 'title', true, 0, 0 );
		}
		...
		if ( ! empty( $this->panel_vars[ CardGridPanel::FIELD_CARDS ] ) ) {
			for ( $i = 0; $i < count( $this->panel_vars[ CardGridPanel::FIELD_CARDS ] ); $i++ ) {
				...
				$options = [
					...
					Card::DESCRIPTION       => $card[ CardGridPanel::FIELD_CARD_DESCRIPTION ],
