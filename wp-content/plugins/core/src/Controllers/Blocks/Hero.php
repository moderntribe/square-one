...
class Hero extends Block_Controller {
	...
	public function render( $attributes, $content, $block_type ) {
		...
		$args = [
			...
			Container::CONTAINER_CLASSES => [ 'hero__container', 'l-container' ],
			Container::MEDIA_CLASSES     => [ 'hero__media' ],
			Container::CONTENT_CLASSES   => [ 'hero__content' ],
			Container::CLASSES           => [ 'c-panel', 'c-panel--hero', 'c-panel--full-bleed' ],
			Container::ATTRS             => [],
		];

		$this->render_component( 'panels/hero/Hero.php', $args );
	}
