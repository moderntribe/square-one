...
class Theme_Provider implements ServiceProviderInterface {
	...
	private function body_classes( Container $container ) {
		...
		add_filter(
			'body_class', function ( $classes ) use ( $container ) {
	...
	private function full_size_gif( Container $container ) {
		...
		add_filter(
			'image_downsize', function( $data, $id, $size ) use ( $container ) {
	...
	private function image_sizes( Container $container ) {
		...
		add_action(
			'after_setup_theme', function () use ( $container ) {
				$container[ 'theme.images.sizes' ]->register_sizes();
			}, 10, 0
		);
		add_filter(
			'wpseo_opengraph_image_size', function ( $size ) use ( $container ) {
	...
	private function image_wrap( Container $container ) {
		...
		add_filter(
			'the_content', function ( $html ) use ( $container ) {
	...
	private function image_links( Container $container ) {
		add_filter(
			'pre_option_image_default_link_type', function () {
				return 'none';
			}, 10, 1
		);
	}
	...
	private function oembed( Container $container ) {
		...
		add_filter(
			'oembed_dataparse', function( $html, $data, $url ) use ( $container ) {
				return $container['theme.oembed']->get_video_component( $html, $data, $url );
			}, 999, 3
		);

		add_filter(
			'embed_oembed_html', function ( $html, $url, $attr, $post_id ) use ( $container ) {
	...
	private function login_resources( Container $container ) {
		...
		add_action(
			'login_enqueue_scripts', function () use ( $container ) {
	...
	private function legacy_resources( Container $container ) {
		...
		add_action(
			'wp_head', function () use ( $container ) {
		...
		add_filter(
			'template_include', function( $template ) use ( $container ) {
	...
	private function fonts( Container $container ) {
		...
		add_action(
			'tribe/unsupported_browser/head', function () use ( $container ) {
				$container[ 'theme.resources.fonts' ]->load_fonts();
			}, 0, 0
		);
		add_action(
			'admin_head', function () use ( $container ) {
				$container[ 'theme.resources.fonts' ]->localize_typekit_tinymce();
			}, 0, 0
		);
		add_filter(
			'mce_external_plugins', function ( $plugins ) use ( $container ) {
	...
	private function scripts( Container $container ) {
		...
		add_action(
			'wp_footer', function () use ( $container ) {
				$container[ 'theme.resources.scripts' ]->add_early_polyfills();
			}, 10, 0
		);
		add_action(
			'wp_enqueue_scripts', function () use ( $container ) {
	...
	private function third_party_tags( Container $container ) {
		...
		add_action(
			'tribe/body_opening_tag', function () use ( $container ) {
	...
	private function editor_styles( Container &$container ) {
		...
		add_filter(
			'tiny_mce_before_init', function ( $settings ) use ( $container ) {
	...
	private function editor_formats( Container &$container ) {
		...
		add_filter(
			'mce_buttons', function ( $settings ) use ( $container ) {
	...
	private function nav_attributes( Container &$container ) {
		...
		add_filter(
			'nav_menu_item_id', function ( $menu_id, $item, $args, $depth ) use ( $container ) {
				return $container[ 'theme.nav.attribute_filters' ]->customize_nav_item_id( $menu_id, $item, $args, $depth );
			}, 10, 4
		);

		add_filter(
			'nav_menu_css_class', function ( $classes, $item, $args, $depth ) use ( $container ) {
				return $container[ 'theme.nav.attribute_filters' ]->customize_nav_item_classes( $classes, $item, $args, $depth );
			}, 10, 4
		);

		add_filter(
			'nav_menu_link_attributes', function ( $atts, $item, $args, $depth ) use ( $container ) {
