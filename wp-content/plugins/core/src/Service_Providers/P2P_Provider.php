...
class P2P_Provider implements ServiceProviderInterface {
	...
	protected function relationships( Container $container ) {
		...
		foreach ( $this->p2p_relationships as $relationship => $sides ) {
			$container[ 'p2p.' . $relationship::NAME ] = function ( $container ) use ( $relationship, $sides ) {
				...
				$from = $this->post_type_is_user( $sides[ 'from' ] ) ? 'user' : array_filter(
					$sides[ 'from' ], [
						$this,
						'is_registered_post_type',
					]
				);
				$to   = $this->post_type_is_user( $sides[ 'to' ] ) ? 'user' : array_filter(
					$sides[ 'to' ], [
						$this,
						'is_registered_post_type',
					]
			...
			add_action(
				'init', function () use ( $container, $relationship ) {
	...
	protected function filters( Container $container ) {
		...
		add_action(
			'tribe_events_pre_get_posts', function ( $query ) use ( $container ) {
				$container[ 'p2p.event_query_filters' ]->remove_event_filters_from_p2p_query( $query );
			}, 10, 1
		);

		add_action(
			'wp_ajax_posts-field-p2p-options-search', function () use ( $container ) {
		...
		add_action(
			'p2p_init', function () use ( $container ) {
		...
		foreach ( [ General_Relationship::NAME ] as $relationship ) {
			...
			add_action(
				'load-post.php', function () use ( $container, $relationship ) {
					$container[ 'p2p.admin_search_filtering.' . $relationship ]->add_post_page_hooks();
				}, 10, 0
			);

			add_action(
				'load-post-new.php', function () use ( $container, $relationship ) {
					$container[ 'p2p.admin_search_filtering.' . $relationship ]->add_post_page_hooks();
				}, 10, 0
			);

			add_filter(
				'p2p_connectable_args', function ( $query_vars, $connection, $post ) use ( $container, $relationship ) {
