...
class Cache_Subscriber extends Abstract_Subscriber {
	...
	protected function listen(): void {
		add_action( 'save_post', function ( ...$args ): void {
			$this->container->get( Listener::class )->save_post( ...$args );
		}, 10, 2 );

		$update_menu = function (): void {
		...
		add_action( 'p2p_created_connection', function ( ...$args ): void {
			$this->container->get( Listener::class )->p2p_created_connection( ...$args );
		}, 10, 1 );

		add_action( 'p2p_delete_connections', function ( ...$args ): void {
