...
class Buttons extends Block_Config implements Cta_Field {
	...
	public function add_block(): void {
		$this->set_block( new Block( self::NAME, [
			'title'       => esc_html__( 'Buttons', 'tribe' ),
			'description' => esc_html__( 'One or more links styled to look like buttons.', 'tribe' ),
			...
			'keywords'    => [ esc_html__( 'formatting', 'tribe' ) ],
			'category'    => 'layout',
			'supports'    => [
				'align'  => false,
				'anchor' => true,
	...
	protected function get_links_section(): Repeater {
		...
		$group->set_attributes( [
			'label'        => esc_html__( 'Buttons', 'tribe' ),
			'name'         => self::BUTTONS,
			'layout'       => 'block',
			'min'          => 1,
			'max'          => 10,
			'button_label' => esc_html__( 'Add Button', 'tribe' ),
		] );
		...
		$button_style = new Field( self::NAME . '_' . self::BUTTON_STYLE, [
			'allow_null'    => 0,
			'choices'       => [
				self:: STYLE_PRIMARY   => esc_html__( 'Primary', 'tribe' ),
				self:: STYLE_SECONDARY => esc_html__( 'Secondary', 'tribe' ),
				self:: STYLE_CTA       => esc_html__( 'Text CTA', 'tribe' ),
			],
			'default_value' => self::STYLE_PRIMARY,
			'label'         => esc_html__( 'Style', 'tribe' ),
			'layout'        => 'horizontal',
