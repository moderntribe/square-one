<?php
/*
Plugin Name: WordPress MU Domain Mapping - Tribe Fork
Plugin URI: http://ocaoimh.ie/wordpress-mu-domain-mapping/
Description: Map any blog on a WordPress website to another domain. Forked from version 0.5.4.2
Version: 9.9.9.9
...
function dm_domains_admin() {
	...
	if ( !empty( $_POST[ 'action' ] ) ) {
		...
		switch( $_POST[ 'action' ] ) {
			...
			break;
			case "save":
				$existing = $wpdb->get_var( $wpdb->prepare( "SELECT domain FROM {$wpdb->dmtable} WHERE blog_id != %d AND domain = %s", $_POST[ 'blog_id' ], $domain ) );

				if ( $_POST[ 'blog_id' ] != 0 && $_POST[ 'blog_id' ] != 1 ) {
					if( !$existing || ( isset( $_POST[ 'orig_domain' ] ) && $existing == $_POST[ 'orig_domain' ] ) ){
	...
	$rows = $wpdb->get_results( "SELECT * FROM {$wpdb->dmtable} ORDER BY id DESC LIMIT $offset,250" );
...
function dm_admin_page() {
	...
	if ( !empty( $_POST[ 'action' ] ) ) {
		...
		if ( $_POST[ 'action' ] == 'update' ) {
			...
			if( $ipok )
				update_site_option( 'dm_ipaddress', $_POST[ 'ipaddress' ] );
			if ( empty( $_POST[ 'always_redirect_admin' ] ) ) {
...
if ( defined( 'DOMAIN_MAPPING' ) ) {
	...
	if ( isset($wp_version) && $wp_version == '2.9.2' ) {
