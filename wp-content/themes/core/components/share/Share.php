...
class Share extends Component {
	...
	public function init() {
		if ( ! isset( $this->data[ self::LABELED ] ) ) {
			$this->data[ self::LABELED ] = true;
		}

		$this->data[ self::LINKS ] = $this->get_links();
	}
	...
	private function build_link( $network, $data ) {
		...
		switch ( $network ) {
			...
			case self::PINTEREST:
				...
				$link       = sprintf( 'http://pinterest.com/pin/create/button/?url=%1$s&amp;media=%2$s&amp;description=%3$s', urlencode( esc_url_raw( $data['link'] ) ), urlencode( esc_url_raw( $data['image_src'] ) ), urlencode( $data['title'] ) );
				$icon       = 'icon-pinterest';
				$attributes = [
			...
			case self::TWITTER:
				...
				$link       = sprintf( 'https://twitter.com/share?url=%1$s&text=%2$s', urlencode( esc_url_raw( $data['link'] ) ), urlencode( $text ) );
				$icon       = 'icon-twitter';
				$attributes = [
					'data-js'     => 'social-share-popup',
					'data-width'  => '550',
			...
			case self::FACEBOOK:
				...
				$link       = sprintf( 'http://www.facebook.com/sharer.php?u=%1$s&t=%2$s', urlencode( esc_url_raw( $data['link'] ) ), urlencode( $data['title'] ) );
				$icon       = 'icon-facebook';
				$attributes = [
					'data-js'     => 'social-share-popup',
					'data-width'  => '640',
			...
			case self::LINKEDIN:
				...
				$link       = sprintf( 'http://www.linkedin.com/shareArticle?mini=true&url=%1$s&title=%2$s', urlencode( esc_url_raw( $data['link'] ) ), urlencode( $data['title'] ) );
				$icon       = 'icon-linkedin';
				$attributes = [
					'data-js'     => 'social-share-popup',
					'data-width'  => '640',
	...
	private function link_component( string $url = '', string $label = '', string $icon = '', array $attributes = [] ): array {
