...
4. You don't *need* to `bash stop.sh` when you stop woking, but it's not a bad idea to do so and to start it again next time, so if someone on the team pushed changes to the infraestructure you'll get them automatically. 
